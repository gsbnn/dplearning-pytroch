...
def just_intime_learning(hist_dataset, test_dataset, batch_size, 
    ...
    for data in create_data_win(test_dataset, win_size):
        ...
        model_train(net, optimizer, loss, similar_data, similar_label, num_epochs, device)
...
if __name__ == '__main__':
    # 超参数
    batch_size = 400
    win_size = 10 # 第5次测试：5，第6次测试：20
    lr = 0.02 # 第5次测试：0.02
    wd = 4.5 # 第5次测试：4.5
    num_epochs = 200 # 第5次测试：170
    dropout = 0.5 # 第5次测试：0.5
    gru_hidden = 128 # 第5次测试：64
    gru_layers = 2
    min_batch = 1
    ...
    test_dataset = get_dataset(test_data_path, select_channels, label)[27*batch_size: 27*batch_size+batch_size]
    batch = len(hist_dataset) // batch_size * min_batch
#   test_dataset = get_dataset(test_data_path, select_channels, label)
#   curret_sample = test_dataset[22*400+390: 22*400+390+win_size]
