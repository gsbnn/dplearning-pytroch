...
def get_similar_data(dataset, batch_size, win_size, curret_data):
    ...
    for data, labels in batch_iter(dataset, batch_size):
        ...
        for win_data in create_data_win(data, win_size):
            distance, path = fastdtw(win_data, curret_data, dist=euclidean)
            distance_list.append(distance)
        distances = torch.tensor(distance_list)
        # 获取相似数据
        index = distances.argsort(dim=0)[0: min_batch] # 每个批次的最近似窗口数据
        for i in index:
            similar_data.append(data[i : i + win_size].t())
            similar_label.append(labels[i : i + win_size])
    #        similar_label.append(labels[i + win_size - 1].unsqueeze(dim=0))
    #        similar_data.append(dataset[i : i + win_size, 1:].t())
    #        similar_label.append(dataset[i + win_size - 1, 0].unsqueeze(dim=0))
...
#        self.gcn3 = GCNCov(in_features, in_features, adjmatrix, dropout)
        self.gru = nn.GRU(varible_num, hidden_fatures, num_layers, 
                          batch_first=True, dropout=dropout)
        self.fc = nn.Sequential(
            nn.Linear(hidden_fatures, 32),
            nn.ReLU(), 
            nn.Dropout(dropout), 
...
#        Y = self.gcn3(self.gcn2(self.gcn1(X)))
...
def model_train(net, X, y, lr, wd, num_epochs, device):
    ...
    for i in range(num_epochs):
        state = net.init_states(device, batch)
        net.train()
        optimizer.zero_grad()
        y_model = net(X, state)
...
#    test_dataset = get_dataset(test_data_path, select_channels, label)[0: batch_size]
    ...
    curret_sample = test_dataset[8*400+250: 8*400+250+win_size]
    min_batch = 2
...
#    print("当前归一化样本：", curret_sample)
    ...
    print("训练标签大小：", similar_label.shape)
...
#    curret_label = curret_sample[-1, 0].unsqueeze(dim=0)
    ...
    error, y_model = model_test_gpu(new_net, curret_data, curret_label)
